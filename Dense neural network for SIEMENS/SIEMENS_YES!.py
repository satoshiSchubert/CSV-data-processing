...
def train_and_val_datagen(data_path, batch_size, whether_train):
	...
	while True:
		for i in range(steps):
			...
			batch_x = np.array([file for file in batch_list[:,4:10]])
			batch_y = np.array([label for label in batch_list[:,13:20]])
...
def Model(num_classes):
    ...
    model.add(Dense(128, activation=tf.nn.leaky_relu))#input_shape=(num_1st_layer_node, )
    model.add(Dense(num_classes, activation=tf.nn.leaky_relu))#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    return model
'''
...
if __name__ == "__main__":
	if TRAIN == True:
		...
		model.compile(loss = tf.keras.losses.Huber(delta=0.6),
		...
		pres = model.predict_generator(generator=testflow_generator,steps=math.ceil(6000/batch_size),verbose=1)
		print(pres.shape)
		
		
		np.savetxt('B_for_test.csv', pres, delimiter = ',')  
